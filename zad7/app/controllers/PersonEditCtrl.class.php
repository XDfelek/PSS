...
class PersonEditCtrl {
	...
	public function validateSave() {
		...
		$this->form->birthdate = getFromRequest('birthdate',true,'Błędne wywołanie aplikacji');
                $this->form->phonenum = getFromRequest('phonenum',true,'Błędne wywołanie aplikacji');
		...
		if ( $d === false ){
			getMessages()->addError('Zły format daty. Przykład: 2015-12-20');
                ...
                $phoneNumValid = "/[+]\d{2}[ ]\d{3}[ ]\d{3}[ ]\d{3}$/i";
                if(!preg_match($phoneNumValid, $this->form->phonenum)){
                    getMessages()->addError('Zły format numeru telefonu. Przykład: +01 123 123 123');
                }
